...
detector_fl, detector_backend, surity = 'face_detector.py', 'retinaface', 3
...
class MainScreen:
    def __init__(self, top=None):
        ...
        # New button for starting live detection
        self.live_detect_btn = Button(root)
        self.live_detect_btn.place(relx=0.06, rely=0.595, width=screen_ratio[0]*250, height=screen_ratio[1]*47)
        self.live_detect_btn.configure(background="#56B235", borderwidth="0", cursor="hand2", text="Start Live Detection", activebackground="#56B235",
                                        font=Font(family="Segoe UI", size=int(0.5*(screen_ratio[0]+screen_ratio[1])*14), weight="bold"),
                                        command=self.toggle_live_detection)
        
        # New button for processing uploaded footage
        self.process_footage_btn = Button(root)
        self.process_footage_btn.place(relx=0.06, rely=0.6815, width=screen_ratio[0]*250, height=screen_ratio[1]*47)
        self.process_footage_btn.configure(background="#F77A39", borderwidth="0", cursor="hand2", text="Process Footage", activebackground="#F77A39",
                                        font=Font(family="Segoe UI", size=int(0.5*(screen_ratio[0]+screen_ratio[1])*14), weight="bold"),
                                        command=self.process_uploaded_footage)
    ...
    def toggle_live_detection(self):
        """Toggle live CCTV detection on/off"""
        if not self.live_detection_active:
            ...
            try:
                ...
                cam_input = messagebox.askquestion("Camera Selection", 
                                                   "Use default camera (0)?\nClick 'No' to enter custom camera ID", 
                                                   parent=root)
                if cam_input == 'no':
                    cam_id_str = simpledialog.askstring("Camera ID", "Enter camera ID (number):", parent=root)
                    if cam_id_str and cam_id_str.isdigit():
                        camera_id = int(cam_id_str)
                    else:
                        messagebox.showerror("Error", "Invalid camera ID", parent=root)
                        return
                
                # Initialize detector if not already done
                if self.detector is None:
                    self.detector = MissingPersonDetector(
                        db_path='data',
                        model_name='arcface',
                ...
                self.detection_thread = Thread(
                    ...
                    args=(camera_id, f"live_cam_{camera_id}")
                )
                self.detection_thread.daemon = True
                self.detection_thread.start()
                
                self.live_detection_active = True
                self.live_detect_btn.configure(text="Stop Live Detection", background="#FF0000")
                messagebox.showinfo("Success", f"Live detection started on camera {camera_id}", parent=root)
                
                # Auto-refresh to show detections
                self.auto_refresh()
                
            except Exception as e:
                logging.error(f"Error starting live detection: {e}")
                messagebox.showerror("Error", f"Failed to start live detection: {str(e)}", parent=root)
        else:
            # Stop live detection
            if self.detector:
                self.detector.stop_detection()
            self.live_detection_active = False
            self.live_detect_btn.configure(text="Start Live Detection", background="#56B235")
            messagebox.showinfo("Success", "Live detection stopped", parent=root)
    
    def process_uploaded_footage(self):
        """Process uploaded CCTV footage"""
        try:
            footage_path = askopenfilename(
                parent=root, 
            ...
            if not path.exists(footage_path):
                messagebox.showerror("Error", "Invalid footage path", parent=root)
                return
            ...
            def process():
                try:
                    messagebox.showinfo("Processing", "Footage processing started. This may take some time.", parent=root)
                    self.detector.process_video(footage_path)
                    messagebox.showinfo("Complete", "Footage processing completed!", parent=root)
                    self.refresh()
                except Exception as e:
                    logging.error(f"Error processing footage: {e}")
                    messagebox.showerror("Error", f"Failed to process footage: {str(e)}", parent=root)
            ...
            logging.error(f"Error in process_uploaded_footage: {e}")
            messagebox.showerror("Error", str(e), parent=root)
    
    def auto_refresh(self):
        """Auto-refresh the display when live detection is active"""
        if self.live_detection_active:
            self.refresh()
            root.after(5000, self.auto_refresh)  # Refresh every 5 seconds
...
class AddMissingPerson:
    ...
    def copyfiles(self, recent_photo_path):
        ...
        # Load image via PIL to ensure RGBA/P formats (like PNG) are normalized to RGB
        try:
            image = Image.open(recent_photo_path)
            if image.mode in ('RGBA', 'P'):
                image = image.convert('RGB')
            
            img_ext = "jpg" # Force jpg extension for safety across pipelines
            img_fl_path = f"./data/{pid}.{img_ext}"
            image.save(img_fl_path, 'JPEG')
        except Exception as e:
    ...
    def start_detection(self, footage_path):
        ...
        # Enhanced: Use the new enhanced detector
        try:
            # Initialize detector
            ...
            thread = Thread(target=detector.process_video, args=(footage_path,))
            thread.daemon = True
            thread.start()
            
            logging.info(f"Started detection for footage: {footage_path}")
        except Exception as e:
